...
(function ($) {
    ...
    BootstrapTable.DEFAULTS = {
        ...
        sortStable: false,
        rememberOrder: false,
        striped: true,// 是否显示行间隔色,默认true显示行间隔色
        columns: [[]],
        data: [],
        totalField: 'total',
        dataField: 'rows',
        method: 'POST',// 数据提交方式默认为post，也可设置为get方式
        url: undefined,
        ajax: undefined,
        cache: false,// 是否开启缓存，默认是false
        ...
        pagination: true,// 是否展示分页，默认为true
        onlyInfoPagination: false,
        paginationLoop: true,
        sidePagination: 'server', //分页方式：client客户端分页，server服务端分页（*）
        totalRows: 0, // server side need to set
        pageNumber: 1,// 初始加载页数
        pageSize: 10,// 每页展示数目，默认每页展示10条记录
        ...
        showHeader: true,
        showFooter: false,
        showColumns: false,// 是否展示可选隐藏的列的操作框，默认为false不展示
        showPaginationSwitch: true,// 是否显示改变分页显示效果的按钮，默认true
        showRefresh: true,// 是否显示刷新按钮，默认为true
        showToggle: true,// 是否显示改变列表的显示效果的按钮，默认为true
        buttonsAlign: 'right',
        smartDisplay: true,
        escape: false,
        minimumCountColumns: 1,// 最少展示记录的列的数目，若低于此列数则不允许通过showColumns按钮进行操作来减少显示的页数
        idField: undefined,
        uniqueId: undefined,// //每一行的唯一标识，一般为主键列
        cardView: false,
        detailView: false,
        ...
        trimOnSearch: true,
        clickToSelect: false,// true单击table某一行的时候可以选中某一行的记录，false则单击table某一行的时候不触发选中某一行记录
        singleSelect: false,// 是否单选，默认false，意思为可以多选
        toolbar: undefined,
        toolbarAlign: 'left',
        checkboxHeader: true,// 是否展示选择框，默认为展示
        sortable: true,
        silentSort: true,
    ...
    BootstrapTable.LOCALES['zh-dl'] = BootstrapTable.LOCALES.en = {
        ...
        formatShowingRows: function (pageFrom, pageTo, totalRows) {
            return sprintf('展示第 %s 到 %s 条记录 总共 %s 条记录', pageFrom, pageTo, totalRows);
        },
        formatDetailPagination: function (totalRows) {
            return sprintf('展示 %s 条', totalRows);
        },
    ...
    $.extend(BootstrapTable.DEFAULTS, BootstrapTable.LOCALES['zh-dl']);
