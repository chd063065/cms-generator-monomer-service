        ...
        for (var i = 0; i < data.length; i++) {
            ...
            if(!data[i].child){
                ...
                if(child.icon!=''&&child.icon!=undefined){
                    $(child_menu_ul_li).append("<a href='javascript:void(0)' ><i class='"+child.icon+"'></i> "+child.name+"</a>")
                    // jquery绑定监听事件的时候同时传递相应的参数【将菜单节点添加到tab标签页上】
                    $(child_menu_ul_li).find("a").bind("click",{id:child.id,name:child.name,url:child.url},function(e){
                        $("#"+opts.tabId).data("tabs").addTab({id: e.data.id, text: e.data.name, closeable: true, url: e.data.url});
                    })
                }else{
                    $(child_menu_ul_li).append("<a href='javascript:void(0)' ><i class='fa fa-fw'></i> "+child.name+"</a>")
                    // jquery绑定监听事件的时候同时传递相应的参数【将菜单节点添加到tab标签页上】
                    $(child_menu_ul_li).find("a").bind("click",{id:child.id,name:child.name,url:child.url},function(e){
                        $("#"+opts.tabId).data("tabs").addTab({id: e.data.id, text: e.data.name, closeable: true, url: e.data.url});
                    })
                ...
                $("#"+opts.treeId).append(child_menu_ul_li);
                continue;
            }
